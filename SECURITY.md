...
| Area | Stato | Dove / come |
|---|---|---|
| CI con soglie bloccanti | ✅ | GitHub Actions (`.github/workflows/ci.yml`), tutto bloccante: sintassi JS, ESLint (`no-undef`, `no-unused-vars`, `no-eval`) su `app.js`, `config.js`, `sw.js` e `rete.js`, validazione HTML, scan segreti, migrazioni applicate da zero **e riapplicate** con i quattro file di test SQL, Playwright sull'anteprima prima del merge e sul sito vivo dopo. Fallisce ⇒ niente merge: `main` è protetta da un ruleset versionato (`.github/rulesets/`) |
